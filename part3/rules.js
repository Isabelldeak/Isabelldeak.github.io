...
function begin_play(){
	var textPlayer1 = document.getElementById("player1_id").value;
	var textPlayer2 = document.getElementById("player2_id").value;
	if(textPlayer1 == "" && textPlayer2 == ""){
		alert("Both Players name were not inputted. Enter Player 1 and Player 2 name");
	}
	else if(textPlayer1 == ""){
		alert("No name inputted for Player 1");
	}
	else if(textPlayer2 == "") {
		alert("No name inputted for Player 2");
	}
	else{
			if (game_started() == true){
			 alert("Game already started. Please Reset Play to start again.");
		 	}
			else{
				started = true;
	 		  var playerX = textPlayer1.concat(" (X)");
	 		  var playerO = textPlayer2.concat(" (O)");
	 		  document.getElementById("player1_id").value = playerX;
	 		  document.getElementById("player2_id").value = playerO;
	 		  document.getElementById("player1_id").disabled = true;
	 		  document.getElementById("player2_id").disabled = true;
				document.getElementById("turn_info").innerText = "Player: X";
			}
...
*/
function displayWinner(player){     //returns player number in boardstate
	if (player == 1){
		alert("Winner is: X");
	}
	else{
...
function checkThreeInRow(a,b,c){

	if(board_state[a] == board_state[b] && board_state[a] == board_state[c] && board_state[a]!= -1){
		displayWinner(board_state[a]);
		return true;
	}
	else{
		return false;
...
function checkWin(){
	if(checkThreeInRow(0,1,2) || checkThreeInRow(3,4,5) || checkThreeInRow(6,7,8) || checkThreeInRow(0,3,6) || checkThreeInRow(1,4,7) || checkThreeInRow(2,5,8)||checkThreeInRow(0,4,8) || checkThreeInRow(2,4,6)){
		return true
	}
	else{
		return false;
...
function updateBoardStatus(move){
		for(var i = 0; i < table_ids.length; i++){
			 if (table_ids[i] == move){
				 if(whose_move() == 1){
					 board_state[i] = 1;
...
function play() {
				...
				var userInput = document.getElementById("move_text_id").value;
				if (game_started() == false){
					alert("The game has not started");
				}
				else if (userInput != "A1" && userInput != "A2" && userInput != "A3" && userInput != "B1"&&userInput != "B2"&&userInput != "B3"&&userInput != "C1" && userInput != "C2" && userInput != "C3"){
						alert("Invalid Move!");
				}
						...
						if(turn == 1){
								...
								if(checkEmpty(userInput) == true){
											document.getElementById(userInput).innerText = "X";
											updateBoardStatus(userInput);
											if (checkWin() == true){
												started = false;
												reset_play();
											}
												...
												toggle_move();
												document.getElementById("turn_info").innerText = "Player: O";
											}

								}
								else{
											alert("Invalid Move. Select an unoccupied square")
								}
								 ...
								 document.getElementById("move_text_id").value = "";
									 ...
									 if(checkEmpty(userInput) == true){
											document.getElementById(userInput).innerText = "O";
											updateBoardStatus(userInput);
											if (checkWin() == true){
												started = false;
												reset_play();
